...
### Node exp display sample ----- 
networkViz_dispSample_Select <- function(sample){
  selectInput("disp_sample", choices = sample, selected = sample[[1]], multiple = FALSE, label = tags$div(
    style = "display: flex; align-items: center;",
    tags$h5("Cluster"),
    tags$div(
      style = "margin-left: 1px;", 
      bsButton("node_Disp_Sample", "", icon = icon("question-circle", class = "fa-lg"), style = "link"), 
      bsPopover("node_Disp_Sample", "Additional Info",
...
networkViz_expColor_Slider <- function(limits){ 
sliderInput("exp_color_scale", min = limits[1], max = limits[2], value = limits, step = 0.1, label = tags$div(
  style = "display: flex; align-items: center;",
  tags$h5("Expression range"),
  tags$div(
    style = "margin-left: 1px;", 
